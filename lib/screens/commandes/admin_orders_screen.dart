...
class _AdminOrdersScreenState extends State<AdminOrdersScreen> {
  ...
  int? _selectedRestaurantId; // Filtre local par restaurant
  int? _selectedMenuId; // Filtre local par menu
  ...
  Future<void> _loadCommandes({bool refresh = false}) async {
    ...
    try {
      // Admin: récupérer TOUTES les commandes d'un coup (pas de pagination)
  ...
  Future<void> _requestRefund(Commande commande, String reason) async {
    try {
      final result = await CommandeService.requestRefund(
        commandeId: commande.id,
        reason: reason,
        token: widget.token,
      );
      ...
      if (result['success']) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Remboursement demandé: ${result['refundId']}'),
    ...
    return Wrap(
      ...
      children: [
        ...
        if (commande.canBeRefunded)
          ElevatedButton.icon(
            onPressed: () => _requestRefund(commande, 'Remboursement demandé par l\'admin'),
        ...
        ElevatedButton.icon(
          onPressed: () => _markAsSuspicious(commande, 'Commande suspecte'),
        ...
        if (commande.canBeCancelled)
          ElevatedButton.icon(
            onPressed: () => _cancelCommande(commande, 'Annulée par l\'admin'),
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildInfoRow('Restaurant ID', commande.restaurantId.toString()),
                              if (commande.userId != null)
                                _buildInfoRow('Utilisateur ID', commande.userId.toString()),
                      ...
                      // Actions admin
                      CommandeViewFactory.buildAdminActions(
                        commande: commande,
                        onRefund: (reason) => _requestRefund(commande, reason),
                      ),
                    ],
                  ),
                ),
